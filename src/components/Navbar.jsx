...
const Navbar = () => {
  ...
  const {menuVisible, setMenuVisible} = useContext(ShopContext)
  ...
  return (
    <div className="flex items-center justify-between py-5 font-medium">
      ...
      <div className="flex items-center gap-6">
        <img src={assets.search_icon} alt="search-icon" className="sm:w-5 w-4 cursor-pointer" />
        ...
        <Link to='/cart' className="relative">
            ...
            <p className="absolute right-[-5px] bottom-[-5px] w-4 text-center leading-4 bg-red-500 text-white aspect-square rounded-full text-[8px]">0</p>
        </Link>
      ...
      <div className={`absolute top-0 right-0 overflow-hidden bg-white transition-transform duration-300 ease-in-out ${menuVisible ? 'w-full' : 'w-0'}`}>
          <div className="flex flex-col text-gray-600">
                ...
                <NavLink className="py-2 pl-6 pt-4 border-b" to='/contact' onClick={() => setMenuVisible(false)}>CONTACT</NavLink>
          </div>
      </div>
